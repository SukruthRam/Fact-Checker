...
public class Spouse
{
    ...
    public boolean getInfo(String s) throws IOException
    {
        ...
        boolean exists = checkForSpouse(s);
    ...
    private boolean checkForSpouse(String s)
    {
        String search3 = "";
        String u = "";
        boolean isExists;
        if(s.contains("spouse is"))
        {
            ////System.out.println(s);
            String retStr[] = changeStr(s);
            u = retStr[1];
            u = u.replaceAll("[.]", "");
            first =  retStr[1].replaceAll("[.]", "");
            first = first.trim();
            search3 = retStr[0];
            search3 = search3.replaceAll("[.]", "");
            second = retStr[0].replaceAll("[.]", "");
            second = second.trim();
        }
        else if(s.contains("spouse."))
        {
        ...
        if (search3.length() > 1)
        {
            ...
            search3 = search3.replace(" ", "%20");
            //System.out.println(search3);
            //search3 = search3.substring(0, search3.length() - 3);
        }
            ...
            try
            {
                if (rev.get(0) != null)
                {
                    ...
                    if (h != null)
                    {
                        ...
                        String result = second.replaceAll("[.]", " ");
                        ...
                        if(a > 0)
                        {
                            ...
                            if(g.contains(result))
                            {
                                ...
                                String newurl = "https://en.wikipedia.org/w/api.php?action=query&prop=revisions&rvprop=content&format=json&titles=" + sear + "&rvsection=0";
    ...
    private String ifKeyword(String[] out, String place)
    {
        ...
        {
            if(out[i].contains(place) || out[i].contains("partner"))
            {
                rtr = rtr + out[i];
